...
export type Opaque<T, TLabel> = T & {
  [__tyinternal_type]: TLabel;
};
/**
 * The argument type of T (which is a function).
 * (In TypeScript 3.2, the spread operator can be inferred)
 */
export type ArgumentType<T> = T extends (arg0: infer U) => Unconstrained
  ? U
 ...
 */
export type ReturnType<T> = T extends () => infer R
  ? R
