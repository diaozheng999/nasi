 ...
 */
import { ensures, requires } from "./Contract.ts";
...
export class Registry<TKey extends string | number | symbol, TValue> {
  ...
  public getValueRecursive = (key: TKey): Option.Type<TValue> => {
    return Option.value_(this.getValue(key), () =>
  ...
  public mapValueRecursive = <T>(
    ...
    return Option.value_(this.mapValue(key, f), () =>
      Option.map(this.fallback[key] as Option.Type<TKey>, (fallback) =>
        this.mapValueRecursive(fallback, f)
      )
    );
  };
  @requires(function (
